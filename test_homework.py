...
zip_file = r"D:\py_projects\lesson7_qaa\resources\archive.zip"
...
def test_pdf(create_zip):
    with (ZipFile(zip_file, "r", ZIP_DEFLATED) as zip_archive):
        ...
        real_pdf = PdfReader("tmp/Python Testing with Pytest (Brian Okken).pdf")
...
def test_xlsx(create_zip):
    with (ZipFile(zip_file, "r", ZIP_DEFLATED) as zip_archive):
        ...
        real_xlsx = load_workbook("tmp/file_example_XLSX_50.xlsx")
...
def test_csv(create_zip):
    with (ZipFile(zip_file, "r", ZIP_DEFLATED) as zip_archive):
        ...
        real_csv = csv.reader(open("tmp/username.csv", 'r'))
